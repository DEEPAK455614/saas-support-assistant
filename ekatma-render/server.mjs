...
async function forward(req,res,body=null,pathOverride=null){const target=pathOverride||req.url;const r=await internalFetch(target,{method:req.method,headers:req.headers,body:body??(req.method==='GET'||req.method==='HEAD'?undefined:await readBody(req))});const b=Buffer.from(await r.arrayBuffer());res.writeHead(r.status,{'content-type':r.headers.get('content-type')||'application/octet-stream','cache-control':r.headers.get('cache-control')||'no-store','x-content-type-options':'nosniff'});res.end(b)}
function safeEq(a,b){const x=Buffer.from(String(a||'')),y=Buffer.from(String(b||''));return x.length===y.length&&timingSafeEqual(x,y)}
function adminOK(req){const expected=process.env.EKATMA_ADMIN_TOKEN||'';return !!expected&&safeEq(expected,req.headers['x-admin-key']||'')}
...
function knowledgeOverview(req,res){if(!adminOK(req))return sendJson(res,401,{error:'unauthorized'});const facts=canonicalFacts.map(f=>({fact_id:f.fact_id,topic:f.topic,question:f.canonical_question,answer:f.canonical_answer,status:f.status,confidence:f.confidence,verified_on:f.verified_on||null,source_ids:f.source_ids||[]}));const sources=Object.entries(sourceRegistry).map(([source_id,s])=>({source_id,...s}));return sendJson(res,200,{stats:canonicalStats(),facts,sources});}
...
setTimeout(()=>gateway.listen(PUBLIC_PORT,'0.0.0.0',()=>console.log(`Ekatma Intelligence OS ${VERSION} on ${PUBLIC_PORT} | conversational Gemini-first | ${canonicalStats().facts} facts / ${canonicalStats().reverseQuestionForms} reverse tests`)),40);
