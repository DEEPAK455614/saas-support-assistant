...
async function searchSources(query,subject=''){const base=await rawInternalFetch(`/api?op=search&q=${encodeURIComponent(query)}`);let j={sources:[]};try{if(base.ok)j=await base.json()}catch{}let qaRows=qaSearch(query,{limit:12,minScore:.18});if(subject){const focused=qaRows.filter(r=>categoryMatchesSubject(r.category,subject));if(focused.length)qaRows=focused}const qa=qaRows.slice(0,8).map(qaResultToSource);return mergeSources(qa,j.sources||[])}
